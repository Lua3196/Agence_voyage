...
import { t as require_client } from "./client-B05GRim-.js";
...
async function createInertiaApp({ id = "app", resolve, setup, title, progress: progress2 = {}, page, render, defaults = {}, nonce, http: http3, layout, strictMode = false, withApp, dev = !!import.meta.env?.DEV } = {}) {
	...
	if (isServer && !page && !render) return async (page2, renderToString) => {
		...
		if (setup) reactApp2 = setup({
			...
			if (withApp) reactApp2 = withApp(reactApp2, {
				ssr: true,
				page: page2
			});
		}
	...
	const reactApp = await Promise.all([resolveComponent(initialPage.component, initialPage), router.decryptHistory().catch(() => {})]).then(([initialComponent]) => {
		...
		if (withApp) appElement = withApp(appElement, {
			ssr: false,
			page: initialPage
		});
...
var Deferred = ({ children, data, rescue, fallback }) => {
	...
	if (!fallback) throw new Error("`<Deferred>` requires a `fallback` prop");
	...
	const rescuedKeys = (0, import_react.useMemo)(() => new Set(page.rescuedProps), [page.rescuedProps]);
	(0, import_react.useEffect)(() => {
	...
	const hasRescuedProps = (0, import_react.useMemo)(() => keys.some((key) => rescuedKeys.has(key)), [keys, rescuedKeys]);
	if (propsAreDefined && !hasRescuedProps) {
		if (typeof children === "function") return children({ reloading });
		return children;
	}
	if (hasRescuedProps && rescue) return typeof rescue === "function" ? rescue({ reloading }) : rescue;
...
function useFormState(options) {
	...
	const setDataFunction = (0, import_react.useCallback)((keyOrData, maybeValue) => {
		if (typeof keyOrData === "string") commitData(set(cloneDeep$1(dataRef.current), keyOrData, maybeValue));
	...
	const reset = (0, import_react.useCallback)((...fields) => {
		...
		if (fields.length === 0) {
			...
			commitData(fields.filter((key) => has(clonedData, key)).reduce((carry, key) => {
